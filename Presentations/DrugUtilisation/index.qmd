...
-   [v1.2.0](https://cran.r-project.org/package=DrugUtilisation) on cran.
...
cdm$drug_exposure <- cdm$drug_exposure |>
  mutate(quantity = as.numeric(sql("[1, 5, 10, 30, 100][floor(random() * 5)::INTEGER + 1]")))
```
...
codelistPara <- getDrugIngredientCodes(cdm = cdm, name = "acetaminophen", nameStyle = "{concept_name}") |>
  ...
  excludeConcepts(cdm = cdm, concepts = 1125315L, codelistName = "acetaminophen")
```
...
cohortCount(cdm$ibuprofen_cohort)
```
...
## Indciations (Mutually exclusive FALSE)
...
```{r}
result <- cdm$dus_cohort |>
  summariseDiscontinuationAsSurvival(followUpDays = 90)
```
