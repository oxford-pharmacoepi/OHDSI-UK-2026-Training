...
**Create a base cohort table named `base_conditions_cohort` that has two cohorts: Rheumatoid Arthritis (RA) and Pneumonia. Use the codelist below.**
...
**2)** **For Pneumonia, assume the infection lasts for two months from the diagnosis day, thereby update the cohort end date to be exactly 60 days after the cohort start.**
...
**Create a third cohort containing patients with Pneumonia who have Rheumatoid Arthritis at the same time. The final cohort table should keep all original cohorts plus the new intersection cohort.**
...
**Finally, rename cohorts so they are called "pneumonia", "rheumatoid_arthritis", and "pneumonia_and_rheumatoid_arthritis".**
